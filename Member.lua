...
function Member:IsGroupHandlingAllowed(name)
  return GroupLib.AmILeader() or self.tMainAssist[name]
end

function Member:IsMemberRaidAssistant(name)
  local count = GroupLib.GetMemberCount()
  for i = 1, count do
    local groupMember = GroupLib.GetGroupMember(i)

    Print("name: " .. tostring(name) .. "groupMember.strCharacterName: " .. groupMember.strCharacterName .. "; groupMember.bRaidAssistant: " .. tostring(groupMember.bRaidAssistant))
    if groupMember and groupMember.bRaidAssistant and groupMember.strCharacterName == name then
      return true
    end
  end
...
function Member:OnQueryBeginDragDrop(wndHandler, wndControl, nX, nY)
  Print("self:IsMemberRaidAssistant(GameLib.GetPlayerUnit()): " .. tostring(self:IsMemberRaidAssistant(GameLib.GetPlayerUnit():GetName())))
  if wndHandler:GetData().parent.editMode or ((self:IsMemberRaidAssistant(GameLib.GetPlayerUnit():GetName()) or GroupLib.AmILeader()) and Apollo.IsAltKeyDown()) then
