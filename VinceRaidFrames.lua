...
function VinceRaidFrames:OnRefresh()
  ...
  for i = 1, count do
    ...
    if groupMember and (groupMember.bIsLeader or groupMember.bRaidAssistant ~= self.tRaidAssistant[groupMember.strCharacterName]) then
      -- leader changed?
      ...
      self.tRaidAssistant[groupMember.strCharacterName] = groupMember.bRaidAssistant
    end
  end
...
function VinceRaidFrames:IsGroupHandlingAllowed(name)
  return self.leader == name or self.tRaidAssistant[name]
end
...
function VinceRaidFrames:OnICCommMessageReceived(channel, strMessage, idMessage)
  ...
  if message.version then
    ...
    if member then
      ...
      if (GroupLib.AmILeader() or self.tRaidAssistant[idMessage]) and not self.settings.tanksHealsDpsLayout then
        self:ShareGroupLayout()
      end
    end
    return
  end
  if self:IsGroupHandlingAllowed(idMessage) then
...
--  local leader = GroupLib.AmILeader()
  local leader = self:IsGroupHandlingAllowed(self.player:GetName())
